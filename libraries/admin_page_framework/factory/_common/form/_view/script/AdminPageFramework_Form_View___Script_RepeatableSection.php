...
class collapsingCategoryList_AdminPageFramework_Form_View___Script_RepeatableSection extends collapsingCategoryList_AdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
     ...
     */
    $.fn.updatecollapsingCategoryList_AdminPageFrameworkRepeatableSections = function( aSettings ) {

        var _oThis                = this; 
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.collapsing.category-list-sections' ).attr( 'id' );

        // Store the sections specific options in an array.
        if ( ! $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableSectionsOptions ) {
            $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableSectionsOptions = [];
        }
        if ( ! $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableSectionsOptions.hasOwnProperty( _sSectionsContainerID ) ) {
            $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ] = $.extend(
                {    
        ...
        var _aOptions = $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ];
        ...
        $( _oThis ).find( '.repeatable-section-add-button' ).click( function() {
            $( this ).addcollapsingCategoryList_AdminPageFrameworkRepeatableSection();
        ...
        $( _oThis ).find( '.repeatable-section-remove-button' ).click( function() {
            $( this ).removecollapsingCategoryList_AdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });     
        
        // If the number of sections is less than the set minimum value, add sections. 
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.collapsing.category-list-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.collapsing.category-list-section' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentSectionCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentSectionCount ) > 0 ) {     
                $( '#' + _sSectionID ).addcollapsingCategoryList_AdminPageFrameworkRepeatableSection( _sSectionID );  
            }
        }
     ...
     */    
    $.fn.addcollapsingCategoryList_AdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {
        
        // Local variables
        if ( 'undefined' === typeof sSectionContainerID ) {
            var sSectionContainerID = $( this ).closest( '.collapsing.category-list-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.collapsing.category-list-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.collapsing.category-list-section-tabs-contents' )
            .children( '.collapsing.category-list-section-tabs' )
            .first();
            
        var _iSectionIndex          = nodeSectionsContainer.attr( 'data-largest_index' );
        
        var _iFadein                = $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadein' ];
        var _iFadeout               = $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadeout' ];
        
        // If the set maximum number of sections already exists, do not add.
        var _sMaxNumberOfSections   = $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'max' ];
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.collapsing.category-list-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.collapsing.category-list-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.collapsing.category-list-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.collapsing.category-list-section-title' ).not( '.collapsing.category-list-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enablecollapsingCategoryList_AdminPageFrameworkCollapsibleButton ){ 
            nodeNewSection.find( '.collapsing.category-list-collapsible-sections-title, .collapsing.category-list-collapsible-section-title' ).enablecollapsingCategoryList_AdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.collapsing.category-list-field' ).each( function( iFieldIndex ) {
            ...
            $( this ).updatecollapsingCategoryList_AdminPageFrameworkRepeatableFields();
                                        
            // Callback the registered callback functions.
            
            // @deprecated 3.8.8 Kept for backward compatibility.
            $( this ).trigger( 
                'collapsing.category-list_added_repeatable_field', 
                [
            ...
            // 3.8.8 
            $( this ).trigger( 
                'collapsing.category-list_repeated_field', 
                [
                    1, // call type, 0: repeatable fields, 1: repeatable sections, 
                    jQuery( nodeNewSection ).closest( '.collapsing.category-list-sections' )    // model container
                ]
            );              
        ...
        nodeNewSection.updatecollapsingCategoryList_AdminPageFrameworkRepeatableSections();    
        
        // Rebind sortable fields - iterate sortable fields containers.
        nodeNewSection.find( '.collapsing.category-list-fields.sortable' ).each( function() {
            $( this ).enablecollapsingCategoryList_AdminPageFrameworkSortableFields();
        });
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.collapsing.category-list-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.collapsing.category-list-fieldrow, .collapsing.category-list-fieldset, .collapsing.category-list-fields, .collapsing.category-list-field, table.form-table, input,textarea,select' )
                .incrementAttribute( 
                    'id', 
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.collapsing.category-list-fields' ).incrementAttribute( 
                'data-field_tag_id_model',
                iSectionsCount,
                _sSectionIDModel
            );
            $( oElement ).find( '.collapsing.category-list-fields' ).incrementAttributes( 
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.collapsing.category-list-fieldset' ).incrementAttribute( 
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
     ...
     */
    $.fn.removecollapsingCategoryList_AdminPageFrameworkRepeatableSection = function() {
        
        // Local variables - preparing to remove the sections container element.
        var nodeSectionContainer    = $( this ).closest( '.collapsing.category-list-section' );
        var sSectionConteinrID      = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.collapsing.category-list-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.collapsing.category-list-section-tabs-contents' )
            .children( '.collapsing.category-list-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.collapsing.category-list-section-tab' );
        ...
        var _sMinNumberOfSections = $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'min' ];
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.collapsing.category-list-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.collapsing.category-list-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() { 
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.collapsing.category-list-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.collapsing.category-list-section-title' ).first().show();
                }                
            }            
...
var _iSectionIndex = _iIterationIndex;
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.collapsing.category-list-field' ).each( function( iFieldIndex ) {    
...
jQuery( document ).ready( function() {
    ...
    // Adds the buttons
    jQuery( '#{$sContainerTagID} .collapsing.category-list-section-caption' ).each( function(){
        
        jQuery( this ).show();
        
        var _oButtons = jQuery( $_sButtonsHTML );
        if ( jQuery( this ).children( '.collapsing.category-list-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
            _oButtons.addClass( 'section_title_field_sibling' );
        }
        var _oCollapsibleSectionTitle = jQuery( this ).find( '.collapsing.category-list-collapsible-section-title' );
    ...
    // Update the fields     
    jQuery( '#{$sContainerTagID}' ).updatecollapsingCategoryList_AdminPageFrameworkRepeatableSections( $_aJSArray ); 
});            
JAVASCRIPTS;
        return "<script type='text/javascript' class='collapsing.category-list-section-repeatable-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
}
