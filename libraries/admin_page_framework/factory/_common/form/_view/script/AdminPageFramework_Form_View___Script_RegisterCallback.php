...
class collapsingCategoryList_AdminPageFramework_Form_View___Script_RegisterCallback extends collapsingCategoryList_AdminPageFramework_Form_View___Script_Base {
    static public function getScript() {
        return <<<JAVASCRIPTS
(function ( $ ) {
            
    // Callback containers.
    $.fn.acollapsingCategoryList_AdminPageFrameworkAddRepeatableFieldCallbacks        = [];            
    $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatFieldCallbacks               = [];    // 3.8.8+
    $.fn.acollapsingCategoryList_AdminPageFrameworkRemoveRepeatableFieldCallbacks     = [];
    $.fn.acollapsingCategoryList_AdminPageFrameworkSortedFieldsCallbacks              = [];            
    $.fn.acollapsingCategoryList_AdminPageFrameworkStoppedSortingFieldsCallbacks      = [];
    $.fn.acollapsingCategoryList_AdminPageFrameworkAddedWidgetCallbacks               = [];
    $.fn.acollapsingCategoryList_AdminPageFrameworkStoppedSortingSectionsCallbacks    = [];    // 3.8.0+
    
    /**
     * Gets triggered when the + (add) button of a repeatable field is pressed.
     */  
    $( document ).bind( 'collapsing.category-list_added_repeatable_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){          
        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.acollapsingCategoryList_AdminPageFrameworkAddRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            ...
            if ( -1 === $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {                
                console.warn( 'Admin Page Framework (' + sFieldType + ' field type): The `added_repeatable_field` callback argument for the `registercollapsingCategoryList_AdminPageFrameworkCallbacks` method is deprecated. Use `repeated_field` instead.' );
            }
     ...
     * 
     * Unlike the `collapsing.category-list_added_repeatable_field` event, this does not call the callback function
     ...
     */
    $( document ).bind( 'collapsing.category-list_repeated_field', function( oEvent, iCallType, oModelContainer ){
        ...
        $.each( $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( -1 !== $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {
                return true;    // continue
     ...
     * @since       3.8.0
     */
    $( document ).bind( 'collapsing.category-list_stopped_sorting_sections', function( oEvent ){  

        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.acollapsingCategoryList_AdminPageFrameworkStoppedSortingSectionsCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
     ...
     */
    /* $( document ).bind( 'collapsing.category-list_removed_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.acollapsingCategoryList_AdminPageFrameworkRemoveRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ];       
            if ( 2 < _aFieldTypes.length && -1 === $.inArray( sFieldType, _aFieldTypes ) ) {
                return true; // continue
    ...
    $.fn.callBackSortedFields = function( sFieldType, sID, iCallType ) {
        var oThisNode = this;
        $.fn.acollapsingCategoryList_AdminPageFrameworkSortedFieldsCallbacks.forEach( function( aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
    ...
    $.fn.callBackStoppedSortingFields = function( sFieldType, sID, iCallType ) {
        var oThisNode = this;
        $.fn.acollapsingCategoryList_AdminPageFrameworkStoppedSortingFieldsCallbacks.forEach( function( aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( 2 < _aFieldTypes.length && -1 === $.inArray( sFieldType, _aFieldTypes ) ) {
                return true; // continue
     ...
     * @since    3.2.0 
     */
    $( document ).bind( 'collapsing.category-list_saved_widget', function( event, oWidget ){
        $.each( $.fn.acollapsingCategoryList_AdminPageFrameworkAddedWidgetCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ];
            if ( ! $.isFunction( _hfCallback ) ) { 
                return true;    // continue
     ...
     */
    $.fn.registercollapsingCategoryList_AdminPageFrameworkCallbacks = function( oCallbacks, aFieldTypeSlugs ) {
        ...
        $.fn.acollapsingCategoryList_AdminPageFrameworkAddRepeatableFieldCallbacks.push( 
            [ oCallbacks.added_repeatable_field, aFieldTypeSlugs ]
        );

        $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatFieldCallbacks.push(  // 3.8.8+
            [ oCallbacks.repeated_field, aFieldTypeSlugs ]
        );        
        $.fn.acollapsingCategoryList_AdminPageFrameworkRemoveRepeatableFieldCallbacks.push( 
            [ oCallbacks.removed_repeatable_field, aFieldTypeSlugs ]
        );
        $.fn.acollapsingCategoryList_AdminPageFrameworkSortedFieldsCallbacks.push( 
            [ oCallbacks.sorted_fields, aFieldTypeSlugs ]
        );
        $.fn.acollapsingCategoryList_AdminPageFrameworkStoppedSortingFieldsCallbacks.push( 
            [ oCallbacks.stopped_sorting_fields, aFieldTypeSlugs ]
        );
        $.fn.acollapsingCategoryList_AdminPageFrameworkAddedWidgetCallbacks.push( 
            [ oCallbacks.saved_widget, aFieldTypeSlugs ]
        );

        // 3.8.0
        $.fn.acollapsingCategoryList_AdminPageFrameworkStoppedSortingSectionsCallbacks.push( 
            [ oCallbacks.stopped_sorting_sections, aFieldTypeSlugs ]
        );        

    };
    /**
     * An alias of the `registercollapsingCategoryList_AdminPageFrameworkCalbacks()` method.
    ...
    $.fn.registerAPFCallback = function( oCallbacks, aFieldTypeSlugs ) {
        $.fn.registercollapsingCategoryList_AdminPageFrameworkCallbacks( oCallbacks, aFieldTypeSlugs );
    }
