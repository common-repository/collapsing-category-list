...
class collapsingCategoryList_AdminPageFramework_Form_View___Script_RepeatableField extends collapsingCategoryList_AdminPageFramework_Form_View___Script_Base {
...
(function ( $ ) {
     ...
     */
    $.fn.updatecollapsingCategoryList_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        if ( ! $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableFieldsOptions ) {
            $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableFieldsOptions = [];
        }
        if ( ! $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( _sFieldsContainerID ) ) {     
            $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ] = $.extend({    
        ...
        var _aOptions = $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ];
...
// @todo For nested fields, the `find()` method should be avoided.
        $( nodeThis ).find( '.collapsing.category-list-repeatable-field-buttons' ).attr( 'data-max', _aOptions[ 'max' ] );
        $( nodeThis ).find( '.collapsing.category-list-repeatable-field-buttons' ).attr( 'data-min', _aOptions[ 'min' ] );
        $( nodeThis ).find( '.collapsing.category-list-repeatable-field-buttons' ).attr( 'data-fadein', _aOptions[ 'fadein' ] );
        $( nodeThis ).find( '.collapsing.category-list-repeatable-field-buttons' ).attr( 'data-fadeout', _aOptions[ 'fadeout' ] );
...
// @todo For nested fields, the `find()` method should be avoided.         
        ...
        $( nodeThis ).find( '.repeatable-field-add-button' ).click( function() {
            $( this ).addcollapsingCategoryList_AdminPageFrameworkRepeatableField();
...
// @todo For nested fields, the `find()` method should be avoided.        
        ...
        $( nodeThis ).find( '.repeatable-field-remove-button' ).click( function() {
            $( this ).removecollapsingCategoryList_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = nodeThis.find( '.repeatable-field-add-button' ).first().closest( '.collapsing.category-list-field' ).attr( 'id' );
        var _nCurrentFieldCount = jQuery( '#' + _sFieldsContainerID ).find( '.collapsing.category-list-field' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addcollapsingCategoryList_AdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addcollapsingCategoryList_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.collapsing.category-list-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.collapsing.category-list-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );

        // If the set maximum number of fields already exists, do not add.
        if ( ! $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( _sFieldsContainerID ) ) {     
            var nodeButtonContainer = nodeFieldContainer.find( '.collapsing.category-list-repeatable-field-buttons' );
            $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ] = {    
        ...
        var _iFadein  = $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadein' ];
        var _iFadeout = $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ];

        // Show a warning message if the user tries to add more fields than the number of allowed fields.
        var sMaxNumberOfFields = $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ]['max'];
        if ( sMaxNumberOfFields != 0 && nodeFieldsContainer.find( '.collapsing.category-list-field' ).length >= sMaxNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.collapsing.category-list-repeatable-field-buttons' ).last();
         ...
         */
        nodeNewField.updatecollapsingCategoryList_AdminPageFrameworkRepeatableFields();
        ...
        $( nodeNewField ).find( '.collapsing.category-list-field' ).addBack().trigger( 
            'collapsing.category-list_repeated_field', 
            [ 
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.collapsing.category-list-fields' )    // model container
            ]
        );
...
// @todo find() may not be appropriate for nested fields.
        ...
        nodeFieldsContainer.children( '.collapsing.category-list-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.collapsing.category-list-field,.collapsing.category-list-fields,.collapsing.category-list-fieldset' ).incrementAttributes(
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removecollapsingCategoryList_AdminPageFrameworkRepeatableField = function() {
        
        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.collapsing.category-list-field' );
        var nodeFieldsContainer = $( this ).closest( '.collapsing.category-list-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        
        /* If the set minimum number of fields already exists, do not remove */
        var sMinNumberOfFields  = $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'min' ];
        if ( sMinNumberOfFields != 0 && nodeFieldsContainer.find( '.collapsing.category-list-field' ).length <= sMinNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.collapsing.category-list-repeatable-field-buttons' ).last();
            ...
            var _iFadeout = $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ]
                ? $.fn.acollapsingCategoryList_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ]
                : 500;
            nodeMessage.delay( 2000 ).fadeOut( _iFadeout );
            return;     
        }     
