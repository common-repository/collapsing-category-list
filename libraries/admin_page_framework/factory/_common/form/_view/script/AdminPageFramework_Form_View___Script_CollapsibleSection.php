...
class collapsingCategoryList_AdminPageFramework_Form_View___Script_CollapsibleSection extends collapsingCategoryList_AdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
    ...
    jQuery( document ).ready( function() {
        jQuery( this ).initializecollapsingCategoryList_AdminPageFrameworkCollapsibleSections();
    });              
     ...
     * @since    3.7.0
     */
    jQuery( document ).bind( 'collapsing.category-list_saved_widget', function( event, oWidget ){
        jQuery( oWidget ).initializecollapsingCategoryList_AdminPageFrameworkCollapsibleSections();
    });    
    
    $.fn.initializecollapsingCategoryList_AdminPageFrameworkCollapsibleSections = function() {
        
        // Expand collapsible sections that are set not to collapse by default 
        jQuery( this ).find( '.collapsing.category-list-collapsible-sections-title[data-is_collapsed=\"0\"]' )
            .next( '.collapsing.category-list-collapsible-sections-content' )
            .slideDown( 'fast' );
        jQuery( this ).find( '.collapsing.category-list-collapsible-section-title[data-is_collapsed=\"0\"]' )
            .closest( '.collapsing.category-list-section-table' )
            .find( 'tbody' )
            .slideDown( 'fast' );
            
        // Hide collapsible sections of 'section' containers as they are somehow do not get collapsed by default.
        jQuery( this ).find( '.collapsing.category-list-collapsible-section-title[data-is_collapsed=\"1\"]' )
            .closest( '.collapsing.category-list-section-table' )
            .find( 'tbody' )
            .hide();
        
        // Bind the click event to the title element.
        jQuery( this ).find( '.collapsing.category-list-collapsible-sections-title, .collapsing.category-list-collapsible-section-title' ).enablecollapsingCategoryList_AdminPageFrameworkCollapsibleButton();
        
        // Insert the toggle all button.
        jQuery( this ).find( '.collapsing.category-list-collapsible-title[data-toggle_all_button!=\"0\"]' ).each( function(){
            
            var _oThis        = jQuery( this ); // to access from inside the below each() method.
            var _bForSections = jQuery( this ).hasClass( 'collapsing.category-list-collapsible-sections-title' );   // or for the 'section' container.
            ...
            jQuery.each( _aPositions, function( iIndex, _sPosition ) {
                ...
                var _sLeftOrRight = -1 !== jQuery.inArray( _sPosition, [ 'top-right', 'bottom-right', '0' ] )   // if found
                    ? 'right'   // default
                    : 'left';            
                _oButton.find( '.collapsing.category-list-collapsible-toggle-all-button' ).css( 'float', _sLeftOrRight );
                ...
                if ( _bForSections ) {
                    var _oTargetElement = 'before' === _sTopOrBottom
                        ? _oThis
                        : _oThis.next( '.collapsing.category-list-collapsible-content' );
                        _oTargetElement[ _sTopOrBottom ]( _oButton );
                } else {    // for 'section' containers
                    _oThis.closest( '.collapsing.category-list-section' )[ _sTopOrBottom ]( _oButton );
                }                
                ...
                _oButton.click( function(){                  
                    ...
                    var _oButtons = _bForSections
                        ? jQuery( this ).closest( '.collapsing.category-list-sectionset' ).siblings().andSelf().find( '> .collapsing.category-list-collapsible-toggle-all-button-container' )
                        : jQuery( this ).siblings( '.collapsing.category-list-collapsible-toggle-all-button-container' ).andSelf();
                    _oButtons.toggleClass( 'flipped' );
                    if ( _bForSections ) {
                        _oButton.parent().parent().children().children( '* > .collapsing.category-list-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    } else {
                        _oButton.closest( '.collapsing.category-list-sections' ).children( '.collapsing.category-list-section' ).children( '.collapsing.category-list-section-table' ).children( 'caption' ).children( '.collapsing.category-list-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    }
     ...
     */
    $.fn.enablecollapsingCategoryList_AdminPageFrameworkCollapsibleButton = function() {
        ...
        jQuery( this ).click( function( event, sContext ){
            ...
            var _sContainerType = jQuery( this ).hasClass( 'collapsing.category-list-collapsible-sections-title' )
                ? 'sections'
                : 'section';
            var _oTargetContent = 'sections' === _sContainerType
                ? jQuery( this ).next( '.collapsing.category-list-collapsible-content' ).first()
            ...
            if ( 'expand' === _sAction && _oThis.data( 'collapse_others_on_expand' ) ) {
                _oThis.parent().parent().children().children( '* > .collapsing.category-list-collapsible-content' ).not( _oTargetContent ).slideUp( 'fast', function() {
                    jQuery( this ).prev( '.collapsing.category-list-collapsible-title' ).addClass( 'collapsed' );
                });
            }
...
JAVASCRIPTS;
    ...
    static private function _getToggleAllButtonHTML($oMsg) {
        ...
        return "<div class='collapsing.category-list-collapsible-toggle-all-button-container'>" . "<span class='collapsing.category-list-collapsible-toggle-all-button button " . $_sDashIconSort . "'" . " title='" . esc_attr($_sLabelToggleAllSections) . "'>" . $_sText . "</span>" . "</div>";
    }
}
