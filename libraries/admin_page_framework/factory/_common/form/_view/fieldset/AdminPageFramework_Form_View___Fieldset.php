...
abstract class collapsingCategoryList_AdminPageFramework_Form_View___Fieldset_Base extends collapsingCategoryList_AdminPageFramework_Form_Utility {
    ...
    private function _getFormatted($aFieldset, $aFieldTypeDefinitions) {
        return $this->uniteArrays($aFieldset, $this->_getFieldTypeDefaultArguments($aFieldset['type'], $aFieldTypeDefinitions) + collapsingCategoryList_AdminPageFramework_Form_Model___Format_Fieldset::$aStructure);
    }
    ...
    private function _loadScripts($sStructureType = '') {
        if ('widget' === $sStructureType && !self::$_bIsLoadedJSScripts_Widget) {
            new collapsingCategoryList_AdminPageFramework_Form_View___Script_Widget;
        ...
        new collapsingCategoryList_AdminPageFramework_Form_View___Script_Utility;
        new collapsingCategoryList_AdminPageFramework_Form_View___Script_OptionStorage;
        new collapsingCategoryList_AdminPageFramework_Form_View___Script_AttributeUpdator;
        new collapsingCategoryList_AdminPageFramework_Form_View___Script_RepeatableField($this->oMsg);
        new collapsingCategoryList_AdminPageFramework_Form_View___Script_SortableField;
    }
...
jQuery( document ).ready( function() {
    var _oButtonPlaceHolders = jQuery( '#{$sFieldsContainerID} > .collapsing.category-list-field.without-child-fields .repeatable-field-buttons' );
    ...
    /* Otherwise, insert the button element at the beginning of the field tag */
         ...
         */
        if ( ! jQuery( '#{$sFieldsContainerID} .collapsing.category-list-repeatable-field-buttons' ).length ) { 
            jQuery( '#{$sFieldsContainerID} > .collapsing.category-list-field.without-nested-fields' ).prepend( $_sSmallButtons );
        }
        /**
         * Support for nested fields.
         * For nested fields, add the buttons to the fields tag.
         */
        jQuery( '#{$sFieldsContainerID} > .collapsing.category-list-field.with-nested-fields' ).prepend( $_sNestedFieldsButtons );
        
        /**
         * Support for inline mixed fields.
         */
        // jQuery( '#{$sFieldsContainerID} > .collapsing.category-list-field.with-mixed-fields' ).prepend( $_sNestedFieldsButtons );
        
    }     
    jQuery( '#{$sFieldsContainerID}' ).updatecollapsingCategoryList_AdminPageFrameworkRepeatableFields( $_aJSArray ); // Update the fields     
});
JAVASCRIPTS;
    ...
    private function _getRepeatableButtonHTML($sFieldsContainerID, array $aSettings, $iFieldCount, $bSmall = true) {
        ...
        $_sDashiconPlus = '';
        $_sDashiconMinus = '';
        return "<div class='collapsing.category-list-repeatable-field-buttons' {$_sSettingsAttributes} >" . "<a class='repeatable-field-remove-button button-secondary repeatable-field-button button {$_sSmallButtonSelector}{$_sDashiconMinus}' href='#' title='{$_sRemove}' {$_sVisibility} data-id='{$sFieldsContainerID}'>" . '-' . "</a>" . "<a class='repeatable-field-add-button button-secondary repeatable-field-button button {$_sSmallButtonSelector}{$_sDashiconPlus}' href='#' title='{$_sAdd}' data-id='{$sFieldsContainerID}'>" . '+' . "</a>" . "</div>";
    }
    protected function _getSortableFieldEnablerScript($sFieldsContainerID) {
        $_sScript = <<<JAVASCRIPTS
    jQuery( document ).ready( function() {
        jQuery( this ).enablecollapsingCategoryList_AdminPageFrameworkSortableFields( '$sFieldsContainerID' );
    });
JAVASCRIPTS;
        return "<script type='text/javascript' class='collapsing.category-list-sortable-field-enabler-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
}
class collapsingCategoryList_AdminPageFramework_Form_View___Fieldset extends collapsingCategoryList_AdminPageFramework_Form_View___Fieldset_Base {
    public function get() {
        $_aOutputs = array();
        $_oFieldError = new collapsingCategoryList_AdminPageFramework_Form_View___Fieldset___FieldError($this->aErrors, $this->aFieldset['_section_path_array'], $this->aFieldset['_field_path_array'], $this->aFieldset['error_message']);
        $_aOutputs[] = $_oFieldError->get();
        $_oFieldsFormatter = new collapsingCategoryList_AdminPageFramework_Form_Model___Format_Fields($this->aFieldset, $this->aOptions);
    ...
    private function _getEachFieldOutput(array $aField, $isIndex, array $aCallbacks, $bIsLastElement = false) {
        ...
        $_oSubFieldFormatter = new collapsingCategoryList_AdminPageFramework_Form_Model___Format_EachField($aField, $isIndex, $aCallbacks, $_aFieldTypeDefinition);
    ...
    private function _getFieldOutput($sContent, array $aField, $bIsLastElement) {
        $_oFieldAttribute = new collapsingCategoryList_AdminPageFramework_Form_View___Attribute_Field($aField);
    ...
    private function _getFinalOutput(array $aFieldset, array $aFieldsOutput, $iFieldsCount) {
        $_oFieldsetAttributes = new collapsingCategoryList_AdminPageFramework_Form_View___Attribute_Fieldset($aFieldset);
    ...
    private function _getEmbeddedFieldTitle(array $aFieldset) {
        ...
        $_oFieldTitle = new collapsingCategoryList_AdminPageFramework_Form_View___FieldTitle($aFieldset, '', $this->aOptions, $this->aErrors, $this->aFieldTypeDefinitions, $this->aCallbacks, $this->oMsg);
    ...
    private function _getChildFieldTitle(array $aFieldset) {
        ...
        $_oFieldTitle = new collapsingCategoryList_AdminPageFramework_Form_View___FieldTitle($aFieldset, array('collapsing.category-list-child-field-title'), $this->aOptions, $this->aErrors, $this->aFieldTypeDefinitions, $this->aCallbacks, $this->oMsg);
        return $_oFieldTitle->get();
    }
    private function _getFieldsetContent($aFieldset, $aFieldsOutput, $iFieldsCount) {
        ...
        $_oFieldsAttributes = new collapsingCategoryList_AdminPageFramework_Form_View___Attribute_Fields($aFieldset, array(), $iFieldsCount);
    ...
    private function _getExtras($aField, $iFieldsCount) {
        $_aOutput = array();
        $_oFieldDescription = new collapsingCategoryList_AdminPageFramework_Form_View___Description($aField['description'], 'collapsing.category-list-fields-description');
