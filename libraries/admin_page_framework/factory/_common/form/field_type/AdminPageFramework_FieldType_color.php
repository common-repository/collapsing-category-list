...
abstract class collapsingCategoryList_AdminPageFramework_FieldType_Base extends collapsingCategoryList_AdminPageFramework_Form_Utility {
    ...
    public function __construct($asClassName = 'admin_page_framework', $asFieldTypeSlug = null, $oMsg = null, $bAutoRegister = true) {
        ...
        $this->oMsg = $oMsg ? $oMsg : collapsingCategoryList_AdminPageFramework_Message::getInstance();
        if ($bAutoRegister) {
    ...
    protected function getFieldOutput(array $aFieldset) {
        ...
        $_oFieldset = new collapsingCategoryList_AdminPageFramework_Form_View___Fieldset($aFieldset, $_oCallerForm->aSavedData, $_oCallerForm->getFieldErrors(), $_oCallerForm->aFieldTypeDefinitions, $_oCallerForm->oMsg, $_oCallerForm->aCallbacks);
    ...
    protected function enqueueMediaUploader() {
        ...
        if (function_exists('wp_enqueue_media')) {
            new collapsingCategoryList_AdminPageFramework_Form_View___Script_MediaUploader($this->oMsg);
        } else {
...
abstract class collapsingCategoryList_AdminPageFramework_FieldType extends collapsingCategoryList_AdminPageFramework_FieldType_Base {
    public function _replyToFieldLoader() {
        $this->setUp();
    }
    public function _replyToGetScripts() {
...
class collapsingCategoryList_AdminPageFramework_FieldType_color extends collapsingCategoryList_AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".repeatable .colorpicker {display: inline;}.collapsing.category-list-field-color .wp-picker-container {vertical-align: middle;}.collapsing.category-list-field-color .ui-widget-content {border: none;background: none;color: transparent;}.collapsing.category-list-field-color .ui-slider-vertical {width: inherit;height: auto;margin-top: -11px;}.collapsing.category-list-field-color .collapsing.category-list-repeatable-field-buttons {margin-top: 0;}.collapsing.category-list-field-color .wp-color-result {margin: 3px;}";
    }
    protected function getScripts() {
        $_aJSArray = json_encode($this->aFieldTypeSlugs);
        $_sDoubleQuote = '\"';
        return <<<JAVASCRIPTS
registercollapsingCategoryList_AdminPageFrameworkColorPickerField = function( osTragetInput, aOptions ) {
    ...
    var _aDefaults = {
        ...
        change: function( event, ui ){
            jQuery( this ).trigger( 
                'collapsing.category-list_field_type_color_changed',
        ...
        clear: function( event, ui ) {
            jQuery( this ).trigger(
                'collapsing.category-list_field_type_color_cleared',
    ...
    jQuery().registercollapsingCategoryList_AdminPageFrameworkCallbacks( {     
        ...
        repeated_field: function( oCloned, aModel ) {
            ...
            oCloned.find( 'input.input_color' ).each( function( iIterationIndex ) {
                ...
                registercollapsingCategoryList_AdminPageFrameworkColorPickerField( _oNewColorInput );                
...
JAVASCRIPTS;
    ...
    protected function getField($aField) {
        ...
        return $aField['before_label'] . "<div class='collapsing.category-list-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'collapsing.category-list-input-label-string') . ">" . $aField['label'] . "</span>" : "") . "<input " . $this->getAttributes($aField['attributes']) . " />" . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "<div class='colorpicker' id='color_{$aField['input_id']}'></div>" . $this->_getColorPickerEnablerScript("{$aField['input_id']}") . "</div>" . $aField['after_label'];
    }
...
jQuery( document ).ready( function(){
    registercollapsingCategoryList_AdminPageFrameworkColorPickerField( '{$sInputID}' );
});            
JAVASCRIPTS;
