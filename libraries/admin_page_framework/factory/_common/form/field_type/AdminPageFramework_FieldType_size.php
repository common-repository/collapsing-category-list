...
class collapsingCategoryList_AdminPageFramework_FieldType_select extends collapsingCategoryList_AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".collapsing.category-list-field-select .collapsing.category-list-input-label-container {vertical-align: top; }.collapsing.category-list-field-select .collapsing.category-list-input-label-container {padding-right: 1em;}";
    }
    protected function getField($aField) {
        $_oSelectInput = new collapsingCategoryList_AdminPageFramework_Input_select($aField['attributes']);
        if ($aField['is_multiple']) {
            $_oSelectInput->setAttribute(array('select', 'multiple'), 'multiple');
        }
        return $aField['before_label'] . "<div " . $this->getLabelContainerAttributes($aField, 'collapsing.category-list-input-label-container collapsing.category-list-select-label') . ">" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . $_oSelectInput->get($aField['label']) . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "</div>" . $aField['after_label'];
    }
}
class collapsingCategoryList_AdminPageFramework_FieldType_size extends collapsingCategoryList_AdminPageFramework_FieldType_select {
    ...
    protected function getStyles() {
        return ".collapsing.category-list-field-size input {text-align: right;}.collapsing.category-list-field-size select.size-field-select {vertical-align: 0px; }.collapsing.category-list-field-size label {width: auto; } .form-table td fieldset .collapsing.category-list-field-size label {display: inline;}";
    }
    protected function getField($aField) {
    ...
    protected function _getFieldOutputByLabel($isKey, $sLabel, array $aField) {
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $isKey, $aField['label']), "<div " . $this->getLabelContainerAttributes($aField, 'collapsing.category-list-input-label-container collapsing.category-list-select-label') . ">", $this->_getNumberInputPart($aField, $_aBaseAttributes, $isKey, is_array($aField['label'])), $this->_getUnitSelectInput($aField, $_aBaseAttributes, $isKey, is_array($aField['label'])), "</div>", $this->getElementByLabel($aField['after_label'], $isKey, $aField['label']));
        return implode('', $_aOutput);
    }
    private function _getNumberInputPart(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_sLabel = $this->getElementByLabel($aField['label'], $isKey, $aField['label']);
        return "<label " . $this->getAttributes($_aSizeLabelAttributes) . ">" . $this->getElement($aField, $bMultiLabels ? array('before_label', $isKey, 'size') : array('before_label', 'size')) . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'collapsing.category-list-input-label-string') . ">" . $_sLabel . "</span>" : "") . "<input " . $this->getAttributes($_aSizeAttributes) . " />" . $this->getElement($aField, $bMultiLabels ? array('after_input', $isKey, 'size') : array('after_input', 'size')) . "</label>";
    }
    private function _getUnitSelectInput(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_oUnitInput = new collapsingCategoryList_AdminPageFramework_Input_select($_aUnitAttributes + array('select' => $_aUnitAttributes));
